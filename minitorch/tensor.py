...
class Tensor:
    ...
    def chain_rule(self, d_output: Any) -> Iterable[Tuple[Variable, Any]]:
        """Apply the chain rule to compute the derivative of the parents"""
        h = self.history
    ...
    # Other operations
    def all(self) -> Tensor:
        """Reshape to 1D tensor and apply All once"""
        flat = self.contiguous().view(self.size)
        return All.apply(flat, self._ensure_tensor(0))
    ...
    # Methods with optional dim argument
    def sum(self, dim: Optional[int] = None) -> Tensor:
        """Compute the sum over dimension"""
        if dim is None:
    ...
    def mean(self, dim: Optional[int] = None) -> Tensor:
        """Compute the mean over dimension"""
        if dim is None:
            return self.sum() / self.size
        else:
            # Convert numpy.int64 to regular Python int
            dim_size = int(self.shape[dim])  # Convert to Python int
            return self.sum(dim) / dim_size

    def view(self, *shape: int) -> Tensor:
        """Returns a new tensor with the same data but different shape"""
        # Convert shape to a tensor with the same backend
